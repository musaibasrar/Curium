...
public class ExamDetailsDAO {
	...
	public List<Examschedule> getExamScheduleDetails(String academicYear,
		...
		try {
			transaction = session.beginTransaction();
			listExamSchedule = session.createQuery("from Examschedule where classes = '"+classH+"' and academicyear = '"+academicYear+"' and examname = '"+exam+"' and branchid="+branchId+" ORDER BY date ASC, STR_TO_DATE(starttime, '%h:%i %p') ASC").list();
			transaction.commit();
		} catch (Exception e) { transaction.rollback(); logger.error(e);
			e.printStackTrace();
		}finally {
