...
public class EmailService {
			...
			if (branchId != null) {
				...
				for (int i = 0; i < iterations; i++) {
					...
					if (!parentsEmails.isEmpty()) {
						...
						if(emailResult.equals(Constants.SUCCESS))	result.setSuccess(true);
	...
	private String sendEmail(String emails, String subject, String message) {
		...
		try {
	        String smtpHost = propertiesUtil.getPropertiesValue("HOSTNAME");
			String smtpPort = propertiesUtil.getPropertiesValue("SMTPPORT");
			String userName = propertiesUtil.getPropertiesValue("USERNAME");
			String password = propertiesUtil.getPropertiesValue("PASSWORD");
			String fromAdd = propertiesUtil.getPropertiesValue("FROMADDRESS");
