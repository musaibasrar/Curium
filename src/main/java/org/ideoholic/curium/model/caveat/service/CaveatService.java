package org.ideoholic.curium.model.caveat.service;
...
import org.ideoholic.curium.model.caveat.dao.CaveatDAO;
...
public class CaveatService {
	...
	public CaveatService(HttpServletRequest request, HttpServletResponse response) {
		this.request = request;
		this.response = response;
		this.httpSession = request.getSession();
	}

	public boolean addCaveats() {
		...
		String caveatTitle = request.getParameter("caveattitle");
		String caveatNo = request.getParameter("caveatno");
		String courtName = request.getParameter("courtname");
		String caveatYear = request.getParameter("caveatyear");
		String caveatDate = request.getParameter("caveatdate");
		String expiryDate = request.getParameter("caveatexpirydate");
		...
		if(httpSession.getAttribute("branchid")!=null){
					...
					Caveat caveats = new Caveat();
					caveats.setCourt(court);
					caveats.setCaveattitle(petitioner+" v/s "+respondent);
					caveatNoDetails = "CAV_"+caveatNo+"_"+caveatYear;
					caveats.setCaveatnumber(caveatNoDetails);
					caveats.setCourtname(courtName);
					caveats.setAssignto(Integer.parseInt(dep[0]));
					caveats.setBranchid(Integer.parseInt(httpSession.getAttribute("branchid").toString()));
					caveats.setCreateddate(DateUtil.indiandateParser(caveatDate));
					caveats.setDateofdispose(DateUtil.indiandateParser(expiryDate));
					caveats.setUpdateddate(new Date());
					caveats.setCreateduserid(Integer.parseInt(httpSession.getAttribute("userloginid").toString()));
					...
					if(resultCaveats!=null && "yes".equalsIgnoreCase(sendQuerySMS)) {
						result = true;
						 String messageClient = "Your File No. "+resultCaveats+" is alloted to "+dep[1]+", Mobile No "+dep[2]+"";
						 String messageInternal = "You are allocated with File No. "+resultCaveats+", client name: "+pidContact[2]+", contact no. "+pidContact[1]+" ";
					...
					}else if(resultCaveats!=null && "no".equalsIgnoreCase(sendQuerySMS)) {
						result = true;
					}
				}
	...
	public boolean viewAllCaveats() {
		...
		if(httpSession.getAttribute("branchid")!=null){
			try {
				...
				List<Caveat> list = new CaveatDAO().readListOfObjectsPagination((page - 1) * recordsPerPage,
						recordsPerPage, Integer.parseInt(httpSession.getAttribute("branchid").toString()));
				int noOfRecords = new CaveatDAO().getNoOfRecords(Integer.parseInt(httpSession.getAttribute("branchid").toString()));
				int noOfPages = (int) Math.ceil(noOfRecords * 1.0 / recordsPerPage);
				request.setAttribute("caveatlist", list);
	...
	public void pendingCaveats() {
		
		String[] caveatsIds = request.getParameterValues("caveatids");
		int userId = Integer.parseInt(httpSession.getAttribute("userloginid").toString());
		List<Integer> caveatsIdsList = new ArrayList<Integer>();
		boolean result = false;
		
		if(caveatsIds!=null) {
			for (String ids : caveatsIds) {
				caveatsIdsList.add(Integer.parseInt(ids));
			}
			
			result = new CaveatDAO().pendingCaveat(caveatsIdsList, userId);
	...
	public void generateCaveatsReportForClient() {
		
		String studentId = request.getParameter("id");
		
		String queryMain = "from Caveat as caveats where caveats.sid = '"+studentId+"'";
		List<Caveat> caveatsQueryList = new ArrayList<Caveat>();
				
		caveatsQueryList = new CaveatDAO().generateCaveatReport(queryMain);
		
		httpSession.setAttribute("caveatlist", caveatsQueryList);
	}


	public void disposeCaveats() {
		
		String[] caveatsIds = request.getParameterValues("caveatsids");
		int userId = Integer.parseInt(httpSession.getAttribute("userloginid").toString());
		...
		if(caveatsIds!=null) {
			...
			Caveat caveats = new Caveat();
			caveats.setFiletaken(fileTaken);
			caveats.setFiletakenby(fileTakenBy);
			caveats.setFiletakenbyname(fileTakenByName);
			caveats.setFiletakenbynumber(fileTakenByContact);
			
			result = new CaveatDAO().disposeCaveat(caveatsIdsList, userId, caveats, disDate);
			request.setAttribute("caveatsstatus",result);
		}
		
	}

	public void noCaveats() {
		...
		if(caveatsIds!=null) {
			...
			result = new CaveatDAO().nocCaveat(caveatsIdsList, userId, caveats, disDate);
			request.setAttribute("caveatsstatus",result);
		}
		
	}

	public void cancelCaveats() {
		...
		if(caveatsIds!=null) {
			...
			result = new CaveatDAO().cancelCaveat(caveatsIdsList, userId);
			request.setAttribute("caveatstatus",result);
		}
		
	}

	public void generateCaveatsReport() {
		...
		String admnno = request.getParameter("clientname");
		String studentName = request.getParameter("studentName");
		String queryMain = "from Caveat as caveats where caveats.createddate between '"+fromDate+"' and '"+toDate+"' ";
		String subQuery = "";
		List<Caveat> caveatsList = new ArrayList<Caveat>();
				
		if(!court.isEmpty()) {
			subQuery = "and caveats.court = '"+court+"' ";
		...
		if(!status.isEmpty()) {
			subQuery = subQuery + " and caveats.status = '"+status+"'";
			httpSession.setAttribute("statusselected", "Status:&nbsp;"+status);
		}else {
		httpSession.setAttribute("statusselected", "");
		}
		...
		if(!studentId.isEmpty()) {
			subQuery = subQuery + "and caveats.sid = '"+studentId+"'";
			httpSession.setAttribute("studentselected", "Client Name:&nbsp;"+studentName);
		}else {
			httpSession.setAttribute("studentselected", "");
		}
		
		caveatsList = new CaveatDAO().generateCaveatReport(queryMain+subQuery);
		
		httpSession.setAttribute("caveatslist", caveatsList);
		httpSession.setAttribute("transactionfromdateselected", "From:"+request.getParameter("transactiondatefrom"));
		httpSession.setAttribute("transactiontodateselected", "To:"+request.getParameter("transactiondateto"));
		
	}

	public void expiredCaveats() {
