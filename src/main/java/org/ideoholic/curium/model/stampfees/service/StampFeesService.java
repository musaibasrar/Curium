...
public class StampFeesService {
	...
	public StampFeesService(HttpServletRequest request, HttpServletResponse response) {
	...
	public void multiClassSearch() {
		...
		if (httpSession.getAttribute(BRANCHID) != null) {
			...
			String querySub = "";
			// String studentname =
			// DataUtil.emptyString(request.getParameter("namesearch"));
			 ...
			 * "%' and parents.Student.branchid="+Integer.parseInt(httpSession.getAttribute(
			 * BRANCHID).toString()); }
			 * 
			 * if (!classStudying.equalsIgnoreCase("") && !querySub.equalsIgnoreCase("")) {
			 ...
			 * classStudying +
			 * "') AND parents.Student.branchid="+Integer.parseInt(httpSession.getAttribute(
			 * BRANCHID).toString())+" order by parents.Student.admissionnumber ASC"; } else
			 * if (!classStudying.equalsIgnoreCase("")) { querySub = querySub +
			 * " (parents.Student.classstudying like '" + classStudying +
			 * "') AND parents.Student.branchid="+Integer.parseInt(httpSession.getAttribute(
			 * BRANCHID).toString())+" order by parents.Student.admissionnumber ASC"; }
			 */

			querySub = " parents.Student.branchid="
					+ Integer.parseInt(httpSession.getAttribute(BRANCHID).toString());
			if (!"".equalsIgnoreCase(querySub)) {
				queryMain = queryMain + querySub;
				searchStudentList = new studentDetailsDAO().getStudentsList(queryMain);
		...
		if (httpSession.getAttribute(CURRENTACADEMICYEAR) != null) {
			...
			if (studentIds != null) {
				...
				for (String id : studentIds) {
					...
					academicfessstructure
							.setUserid(Integer.parseInt(httpSession.getAttribute("userloginid").toString()));
				...
				for (String id : studentIds) {
					...
					for (int i = 0; i < feesCategoryIds.length; i++) {
						...
						studentfeesstructure
								.setBranchid(Integer.parseInt(httpSession.getAttribute(BRANCHID).toString()));
				...
				int crFees = getLedgerAccountId(
						"unearnedstudentfeesincome" + Integer.parseInt(httpSession.getAttribute(BRANCHID).toString()));
				int drAccount = getLedgerAccountId(
						"studentfeesreceivable" + Integer.parseInt(httpSession.getAttribute(BRANCHID).toString()));
				;
				...
				transactions.setFinancialyear(new AccountDAO()
				...
				String updateDrAccount = "update Accountdetailsbalance set currentbalance=currentbalance+" + grandTotal
						+ " where accountdetailsid=" + drAccount;

				String updateCrAccount = "update Accountdetailsbalance set currentbalance=currentbalance+" + grandTotal
						+ " where accountdetailsid=" + crFees;

				// End J.V
				new StampFeesDAO().addStampFees(listOfacademicfessstructure,
						httpSession.getAttribute(CURRENTACADEMICYEAR).toString(), listOfstudentfeesstructure,
