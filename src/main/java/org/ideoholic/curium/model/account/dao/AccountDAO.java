...
public class AccountDAO {
	...
	public List<Accountdetailsbalance> getAccountdetailsbalanceBankCash(int branchId) {
		...
		try{
			transaction = session.beginTransaction();
			accountDetails = session.createQuery("from Accountdetailsbalance as accdetails where accdetails.accountDetails.accountSSGroupMaster.ssgroupname IN ('Cash In Hand', 'Cash In Bank') and branchid="+branchId).list();
			transaction.commit();																						   											
		}catch (Exception hb) { transaction.rollback(); logger.error(hb);
			hb.printStackTrace();
		}finally {
