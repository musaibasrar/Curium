...
public interface VoucherEntryTransactionsRepository extends JpaRepository<VoucherEntrytransactions, Integer> {
    ...
    @Query("SELECT v FROM VoucherEntrytransactions v " +
        "WHERE v.transactiondate BETWEEN :fromDate AND :toDate " +
        "AND (v.draccountid = :accNo OR v.craccountid = :accNo) " +
        "AND v.cancelvoucher != 'yes' " +
        "AND v.branchid = :branchId " +
        "ORDER BY v.transactionsid ASC")
    List<VoucherEntrytransactions> findTransactionsBetweenDates(
        @Param("fromDate") Date fromDate,
        @Param("toDate") Date toDate,
        @Param("accNo") int accNo,
