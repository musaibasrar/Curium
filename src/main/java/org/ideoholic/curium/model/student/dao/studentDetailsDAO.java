...
public class studentDetailsDAO {
	...
	public List<Object[]> readListOfObjectsPagination(int offset, int noOfRecords, int branchId) {
		...
		try {
			 ...
			 */
			Query query = session
					.createQuery("select s.sid, s.studentexternalid, s.admissionnumber, s.name, s.classstudying, s.sts, s.branchid, f.fathersname, f.mothersname from Student s JOIN Parents f ON s.sid=f.Student.sid where s.archive = 0 AND s.branchid="+branchId+" order by s.sid DESC").setCacheable(true).setCacheRegion("commonregion");
