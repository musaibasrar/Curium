...
<script type="text/javascript">
	...
	function saveInventory() {
		...
		if(form1.checkValidity()) {
			form1.action = "/oasis/MessItemsProcess/savePurchase";
			form1.method = "POST";
			form1.submit();
		}
	...
	function cancelPurchase() {
				...
				var form1 = document.getElementById("form1");
				form1.action = "/oasis/MessItemsProcess/cancelPurchase";
				form1.method = "POST";
				form1.submit();
...
<script type="text/javascript">
            ...
            function openPopup(invoicedetailsid,date, supplierrefno, name, invoicetotal){
        			...
        			xmlHttp.open("GET", "/oasis/stockentry/mrvDetails?invoicedetailsid="+invoicedetailsid+"&entrydate="+date+"&supplierreferenceno="+supplierrefno+"&suppliername="+name+"&invoicetotal="+invoicetotal+"",true);
        			xmlHttp.send(null);
			...
			<table width="100%" border="0" style="border-color: #4b6a84;"
             ...
             <%--For displaying Previous link except for the 1st page --%>
                <c:if test="${currentPage != 1}">
                    <td><a style="color: #4B6A84;font-size: 12px" href="/oasis/MessItemsProcess/purchaseItems?page=${currentPage - 1}">Previous</a></td>
                </c:if>
                    ...
                    <tr>
                        <c:forEach begin="1" end="${noOfPages}" var="i">
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <td style="color: black;font-weight:bold;font-size: 15px "><a style="color: #4B6A84" href="/oasis/MessItemsProcess/purchaseItems?page=${i}">${i}</a></td>
                                </c:otherwise>
                            </c:choose>
                        </c:forEach>
                    </tr>
                ...
                <c:if test="${currentPage lt noOfPages}">
                    <td ><a style="color: #4B6A84;font-size: 12px" href="/oasis/MessItemsProcess/purchaseItems?page=${currentPage + 1}">Next</a></td>
                </c:if>
                    </div>
