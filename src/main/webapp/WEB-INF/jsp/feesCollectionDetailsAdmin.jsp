	...
	<script type="text/javascript">
				...
				var updatereceiptpaymentmethod = '<c:out default="" value="${updatereceiptpaymentmethod}"/>';
		            ...
		            function submitfees(cashpayment,banktransfer,chequetransfer,ackno,transferdate,transferbankname,chequeno,chequedate,chequebankname){
		            	...
		            	var form1 = document.getElementById("form1");
		        		form1.action="/gnyanganga/FeesCollection/feesPaymentTypeModify?receiptnumber="+receiptnumber+"&paymentmethod="+paymentmethodvalue+"&ackno="+acknovalue+"&transferdate="+transferdatevalue+"&transferbankname="+transferbanknamevalue+"&chequeno="+chequenovalue+"&chequedate="+chequedatevalue+"&chequebankname="+chequebanknamevalue+"";
		        		form1.method = "POST";
		        		form1.submit();
                    ...
                    <tbody>
                        <c:forEach items="${searchfeesdetailslist}" var="feesdetails">
                            ...
                            <tr class="trClass" >
                                ...
                                <td  class="dataTextInActive"><a class="dataTextInActive" href="/gnyanganga/FeesCollection/CancelFeesReceipt?id=<c:out value='${feesdetails.key.receiptnumber}'/>&sid=<c:out value='${feesdetails.key.sid}'/>&receiptid=<c:out value='${feesdetails.key.receiptvoucher}'/>&journalid=<c:out value='${feesdetails.key.journalvoucher}'/>&fromdate=<c:out value='${datefrom}'/>&todate=<c:out value='${dateto}'/>"><i class="fa fa-times" style="color:#93051f;font-size: 18px;"></i></a></td>
                            </tr>
                        </c:forEach>
                    </tbody>
