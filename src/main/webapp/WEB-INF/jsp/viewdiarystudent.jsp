...
<script type="text/javascript">
  ...
  function viewStudentDiary(sid) {
    ...
    form1.action = "/shadaan/StudentDiaryProcess/ViewDiaryDetailsParent?id=" + sid;
    form1.submit();
  }
  ...
  <div class="main-content">
    <%-- <table class="table table-striped table-bordered table-hover" id="myTable">
      ...
      <c:if test="${currentPage != 1}">
        <a class="btn btn-primary" href="/shadaan/DiaryProcess/viewdiarystudent?page=${currentPage - 1}">Previous</a>
      </c:if>
      <ul class="pagination">
        <c:forEach begin="1" end="${noOfPages}" var="i">
          <li class="page-item <c:if test='${currentPage == i}'>active</c:if>">
            <a class="page-link" href="/shadaan/DiaryProcess/viewdiarystudent?page=${i}">${i}</a>
          </li>
        </c:forEach>
      </ul>
      <c:if test="${currentPage lt noOfPages}">
        <a class="btn btn-primary" href="/shadaan/DiaryProcess/viewdiarystudent?page=${currentPage + 1}">Next</a>
      </c:if>
    </div> --%>
  </div>
</div>
