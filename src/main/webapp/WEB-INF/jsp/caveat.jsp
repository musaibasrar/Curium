...
<script type="text/javascript" src="/sla/js/datetimepicker_css.js"></script>
	...
	<script type="text/javascript">
		function pendingCaveats() {
			var form1 = document.getElementById("form1");
			form1.action = "/sla/CaveatProcess/pendingCaveats";
			form1.method = "POST";
			form1.submit();
		}
		
		function expiredCaveats() {
			var form1 = document.getElementById("form1");
			form1.action = "/sla/CaveatProcess/expiredCaveats";
			form1.method = "POST";
			form1.submit();
		}
		
		function cancelCaveats() {
			var form1 = document.getElementById("form1");
			form1.action = "/sla/CaveatProcess/cancelCaveats";
			form1.method = "POST";
			form1.submit();
		}
	 ...
	 <script type="text/javascript">
					...
					var caveatstatus = '<c:out default="" value="${caveatstatus}"/>';
		            
		            if(caveatstatus == "true"){
		            	 $(function(){
		            		 $( "div.success" ).fadeIn( 800 ).delay( 2000 );
		            		 $( "div.success" ).fadeOut("slow");
		            	 });
		            	 }else if(caveatstatus == "false"){
		            	  $(function(){
...
<script type="text/javascript">
 ...
 <script type="text/javascript">
        	...
        	function dispose(disposedate,disposefiletaken,disposefiletakenby,disposefiletakenbyname,disposefiletakenbycontact){
            	...
            	var form1 = document.getElementById("form1");
        		form1.action = "/sla/CaveatProcess/addDispose?disposedate="+ddate+"&disposefiletaken="+fileTaken+"&disposefiletakenby="+fileTakeBy+"&disposefiletakenbyname="+fileTakenByName+"&disposefiletakenbycontact="+fileTakenByContact+"";
        		form1.method = "POST";
        		form1.submit();
            }
			...
			function noc(nocdate,nocfiletaken,nocfiletakenby,nocfiletakenbyname,nocfiletakenbycontact){
            	...
            	var form1 = document.getElementById("form1");
        		form1.action = "/sla/CaveatProcess/addNOC?nocdate="+ddate+"&nocfiletaken="+fileTaken+"&nocfiletakenby="+fileTakeBy+"&nocfiletakenbyname="+fileTakenByName+"&nocfiletakenbycontact="+fileTakenByContact+"";
        		form1.method = "POST";
        		form1.submit();
            }
        ...
        <script type="text/javascript">
            ...
            function openPopup(referredby){
        			...
        			xmlHttp.open("GET", "/sla/CaveatProcess/viewReferredby?referredby="+referredby+"",true);;
        			xmlHttp.send(null);
...
<body>
	<form id="form1" method="post">
		...
		<div class="alert-box success" id="div1">Caveat updated successfully!!!&nbsp;&nbsp;&nbsp;<button class="button" id="1" onclick="closediv(this.id);">OK</button></div>
		<div class="alert-box failure" id="div2">Caveat update failed, please try again!!!&nbsp;&nbsp;&nbsp;<button class="buttonred" id="2" onclick="closediv(this.id);">OK</button></div>
		
		<div style="overflow: scroll; height: 600px">
			<table width="100%">
				<tr>
					<td class="headerTD">Caveats</td>
                    ...
                    <tbody>
                        <c:forEach items="${caveatlist}" var="caveat">
                            <tr class="trClass" style="border-color:#000000" border="1"  cellpadding="1"  cellspacing="1" >
                            	...
                            	<td class="dataText"><input type="checkbox" id = "<c:out value="${caveat.id}"/>" class = "chcktbl"  name="caveatids"  value="<c:out value="${caveat.id}"/>"/></td>
                            	<td class="dataText"><c:out value="${caveat.id}"/><input type="hidden" id="username" name="username" value="${username}"> </td></td>
                            	<c:if test="${caveat.status == 'Pending' }">
                                	<td class="dataText" style="color: #0001ff;font-weight: bold;">
                                	<a class="dataTextInActive" style="color: #0001ff;font-weight: bold;cursor: pointer;text-transform: capitalize;" onclick="viewStudentDetails(${caveat.sid},${caveat.branchid})">${caveat.caveattitle}</a>
                                	</td>
                                </c:if>
                                <c:if test="${caveat.status == 'Expired' }">
                                	<td class="dataText" style="color: red;font-weight: bold;">
                                		<a class="dataTextInActive" style="color: red;font-weight: bold;cursor: pointer;text-transform: capitalize;" onclick="viewStudentDetails(${caveat.sid},${caveat.branchid})">${caveat.caveattitle}</a>
                                	</td>
                                </c:if>
                                <c:if test="${caveat.status == 'Cancelled' }">
                                	<td class="dataText" style="color: grey;font-weight: bold;">
                                		<a class="dataTextInActive" style="color: grey;font-weight: bold;cursor: pointer;text-transform: capitalize;" onclick="viewStudentDetails(${caveat.sid},${caveat.branchid})">${caveat.caveattitle}</a>
                                	</td>
                                </c:if>
                               ...
                               <td class="dataText"><c:out value="${caveat.fileno}"/></td>
                              	<td class="dataText"><c:out  value="${caveat.status}"/></td>
                              		<c:choose>
									<c:when test="${caveat.court == 'DC'}">
										<td class="dataText"><c:out  value="${caveat.courtname}"/></td>
									</c:when>
									<c:when test="${caveat.court == 'RC'}">
										<td class="dataText"><c:out  value="${caveat.courtname}"/></td>
									</c:when>
									<c:otherwise>
								...
								<td class="dataText"><a href="#" onclick="openPopup('<c:out value="${caveat.referredby}"/>')" style="color:#eb6000;">View Details</a></td>
                            </tr>
                        </c:forEach>
                    </tbody>
                ...
                <div align="center">
		             <%--For displaying Previous link except for the 1st page --%>
		                <c:if test="${currentPage != 1}">
		                    <td><a style="color: #4B6A84;font-size: 12px" href="/sla/CaveatProcess/viewAllCaveats&page=${currentPage - 1}">Previous</a></td>
		                </c:if>
		                ...
		                <table border="0" cellpadding="2" cellspacing="2">
		                	<c:forEach begin="1" end="${noOfPages}" var="i" varStatus="status">
		                            ...
		                            <c:choose>
		                                ...
		                                <c:otherwise>
		                                    <td style="color: black;font-weight:bold;font-size: 15px "><a style="color: #4B6A84" href="/sla/CaveatProcess/viewAllCaveats&page=${i}">${i}</a></td>
		                                </c:otherwise>
		                            </c:choose>
		                ...
		                <c:if test="${currentPage lt noOfPages}">
		                    <td ><a style="color: #4B6A84;font-size: 12px" href="/sla/CaveatProcess/viewAllCaveats&page=${currentPage + 1}">Next</a></td>
		                </c:if>
                </div>
		</div>
