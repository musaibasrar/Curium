...
%>
	...
	font-weight: bolder;
	color: #5E87B0;
        font-size: 20px;"> SUBJECT PERFORMANCE GRAPH &nbsp;&nbsp;&nbsp;<br><br>Subject: ${subjectName}</label><br><br>
    		<!-- <h1 align="center" class="headerTD">Number Of Students Per Center</h1> -->
			<%-- <canvas id="canvas" height="100" width="300"></canvas> --%>
			
			<c:forEach items="${subjectaveragelist}" var="subjectaveragelist" varStatus="status">
   ...
   <label id="subjectaveragelistsize" style="display: none;">${subjectaveragelistsize}</label>
	
	
	<c:forEach items="${subjectaveragelist}" var="subjectaveragelist" varStatus="status">
		<label id="subjectname${status.index}" style="display: none;">${subjectaveragelist.classsec}</label>
		<label id="marksobtained${status.index}" style="display: none;">${subjectaveragelist.averageMarks}</label>
		<label id="examname${status.index}" style="display: none;">${subjectaveragelist.examName}</label>
	</c:forEach>
	     ...
	     <script>
	     var i=0;
	     var ii =document.getElementById("subjectaveragelistsize").innerHTML;
