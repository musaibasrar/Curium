...
<script type="text/javascript" src="/readmodelschool/js/globalize.min.js"></script>
<script type="text/javascript" charset="utf-8" src="/readmodelschool/js/globalize.cultures.js"></script>
...
<script type="text/javascript">
	function searchForFees() {
		...
		form1.action = "/readmodelschool/StampFeesProcess/advanceSearchForStampsingleFees";
		form1.method = "POST";
		form1.submit();
	...
	function deleteFeesStamp(){
		var form1 = document.getElementById("form1");
		form1.action = "/readmodelschool/StampFeesProcess/delete";
		form1.method = "POST";
		form1.submit();
	}
</script>
    ...
    function updateFeesCount(rowvalue){
	 ...
	 function toggleFeesCount(allCheckbox) {
      ...
      $(document).ready(function() {
    	    ...
    	    $('.amountfees').on('input', function() {

    	        var amount = $.trim($(this).val());

    	        // Find checkbox in the same row
    	        var checkbox = $(this).closest('tr').find('.chcktbl');
...
%>
<body>
	<form id="form1" action="/readmodelschool/StampFeesProcess/applySingleFees" method="POST">
    ...
    type="radio"
    name="feesIDS"
    id="feesIDS_${status.index+1}"
    class="chcktblStamp"
    ${status.index == 0 ? 'checked="checked"' : ''}
    value="${feescategory.idfeescategory}_${status.index}"
    onclick="updateFeesCount(${status.index+1});calculate(${status.index+1})"> ${feescategory.feescategoryname} :
											</label> <input type="hidden" class="feesStatus" name="feesStatuses"
												id="fees_status_${status.index+1}" value="not set" />
												<input
					...
					<c:forEach items="${searchStudentList}" var="Parents" varStatus="status">
						...
						<tr class="trClass" style="border-color: #000000" border="1"
							...
							<td class="dataText"><input type="checkbox"
								...
								name="studentIDs"
								value="<c:out value="${Parents.student.sid}_${status.index}"/>" /></td>
								<td class="dataText" align="center">${status.index + 1}</td>
							<td class="dataTextInActive"><c:out
										value="${Parents.student.studentexternalid}" /></td>
							<td class="dataTextInActive">
							<input type="text" name="oldfeesamount" class="amountfees" style="width:80px;"  /></td>
