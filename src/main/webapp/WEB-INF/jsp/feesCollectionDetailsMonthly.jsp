...
<script type="text/javascript">
	function searchFeesCollectionDetailsMonthly() {
		var form1 = document.getElementById("form1");
		form1.action = "/jih/FeesCollection/searchFeesCollectionDetailsMonthly";
		form1.method = "POST";
		form1.submit();
	...
	$(function() {
		...
		$("#search").button().click(function() {
			searchFeesCollectionDetailsMonthly();
		});
		$("#effect").hide();
...
<body>
	<form id="form1"
		action="/jih/FeesCollection/exportDataForFeesMonthlyCollection" method="POST">
					...
					<table width="100%" border="0" align="center" cellpadding="0"
						...
						<td width="20%" class="alignRight">From Date(MM/DD/YYYY)&nbsp;</td>
							...
							<td width="28%"><label> <input name="todate" autocomplete="off"
							 ...
							 <input type="hidden" name="fromdateselected" value="${fromdateselected}" />
    						 <input type="hidden" name="todateselected" value="${todateselected}" />
		...
		<!-- Monthly Fees Collection Table -->
        <div style="overflow: auto; margin-top: 0px;">
       			 <table width="100%">
						<tr>
							<td class="headerTD"><label style="color: #EB6000;">Search Result</label></td>
						</tr> 
					</table>
            <table border="1" width="100%" style="border-collapse: collapse;">
                <thead>
                    <tr>
                        ...
                        <c:forEach var="month" items="${monthKeys}">
                            <th class="headerText">${month}</th>
                        </c:forEach>
                    </tr>
                </thead>
                <tbody>
                    <c:forEach var="student" items="${studentMonthlyFeesList}">
                        <tr>
                            <td class="dataText">${student.studentName}</td>
                            <c:forEach var="month" items="${monthKeys}">
                                <td class="dataText">
                                    <c:out value="${student.monthlyFees[month]}"/>
                                </td>
                            </c:forEach>
                        </tr>
                    </c:forEach>
