...
<script type="text/javascript">
	...
	function getTrialBalance(){
		...
		form1.action = "/jihtel/AccountProcess/trialBalance";
		form1.method = "POST";
		form1.submit();
	}
...
function printTrialBalance(){
	var form1 = document.getElementById("form1");
	form1.action = "/jihtel/AccountProcess/printTrialBalance";
	form1.method = "POST";
	form1.submit();
}
...
function exportTrialBalance(){
	var form1 = document.getElementById("form1");
	form1.action = "/jihtel/AccountProcess/exportTrialBalance";
	form1.method = "POST";
	form1.submit();
}
</script>
		...
		<table width="100%">
			...
			<table width="100%" border="0" style="border-color: #4b6a84;" id="myTable">
				...
				<tbody>
					...
					<c:forEach items="${accountdetailsbalanceMap}" var="accountdetails">
						...
						<tr class="trClass" style="border-color: #000000" border="1"
							...
							<a target="mainFrame" href="/jihtel/AccountProcess/searchSingleLedgerEntries?accountid=${accountdetails.key.accountdetailsid}&ledgername=${accountdetails.key.accountname}">
							...
							<c:if test="${(accountdetails.key.accountGroupMaster.accountgroupid == 2) || (accountdetails.key.accountGroupMaster.accountgroupid == 3) || (accountdetails.key.accountGroupMaster.accountgroupid == 4)}">
									...
									<c:if test="${accountdetails.value >= 1}">
										<td class="dataText"></td>
										<td class="dataText" style="text-align: right;">
										<fmt:formatNumber type="currency" pattern="#,##0.00;" value="${accountdetails.value}" /></td>			
									</c:if>

									<c:if test="${accountdetails.value < 1}">
										<td class="dataText" style="text-align: right;">
										<fmt:formatNumber type="currency" pattern="#,##0.00;" value="${accountdetails.value*-1}" /></td>
										<td class="dataText"></td>			
									</c:if>
				...
				<tr class="trClass" style="border-color: #000000" border="1"
							...
							<td class="dataTextInActive" style="text-align: right">
							<fmt:formatNumber type="currency" pattern="#,##0.00;" value="${debittotal}" /></td>
							<td class="dataTextInActive" style="text-align: right">
							<fmt:formatNumber type="currency" pattern="#,##0.00;" value="${credittotal}" /></td>
						</tr>
					<tr>
