...
public class studentDetailsDAO {
	...
	public List<Pudetails> getPuDetails(long id) {
		List<Pudetails> results = new ArrayList<Pudetails>();
		...
		try {
			...
			// session.createQuery("From PersonalDetails p where p.subscriber=1 and  p.archive = 0 order by name desc LIMIT 5 ").list();
			Query query = session
					.createQuery("FROM Pudetails pu where pu.exampassedappearance ="+id+" order by pu.idpudetails desc").setCacheable(true).setCacheRegion("commonregion");
			results = query.list();
			transaction.commit();
