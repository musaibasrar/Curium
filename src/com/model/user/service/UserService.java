...
public class UserService {
	...
	public void dashBoard() {
		...
		if (httpSession.getAttribute(BRANCHID) != null) {
			 ...
			 * 
			 * if (!classStudying.equalsIgnoreCase("")) { classStudying = classStudying+"--"
			 * +"%"; }
			 * 
			 * 
			 * List<Parents> student = new
			 * studentDetailsDAO().getStudentsList("FROM Parents as parents where" +
			 * " parents.Student.archive=0 AND parents.Student.passedout=0 AND parents.Student.droppedout=0 AND parents.Student.leftout=0"
			 * ); totalStudents+=student.size();
			 * 
			 * xaxisList.add("\""+classstudying.getClassdetails()+"\"");
			 * if(student.size()>0) { String studentCount =
			 * Integer.toString(student.size()); yaxisList.add("\""+studentCount+"\"");
			 * }else { yaxisList.add("\""+0+"\""); }
			 * 
			 * 
			 * }
			 */
			
			   List<Parents> student = new studentDetailsDAO().getStudentsList("FROM Parents as parents where"
               		+ " parents.Student.archive=0 AND parents.Student.passedout=0 AND parents.Student.droppedout=0 AND parents.Student.leftout=0");
               totalStudents+=student.size();
	...
	public void feesdailysearch() {
		...
		for (Date date = start.getTime(); start.before(end); start.add(Calendar.DAY_OF_MONTH,
	...
	public void searchByDate() {
		...
		if (httpSession.getAttribute(BRANCHID) != null) {
			...
			String queryMain = "From Receiptinfo as feesdetails where feesdetails.cancelreceipt=0 and feesdetails.branchid="
					+ idBranch + " AND";
			...
			if (fromDate != null && toDate != null) {
				...
				querySub = " feesdetails.date between '" + (String) httpSession.getAttribute("datefrom") + "' AND '"
						+ (String) httpSession.getAttribute("dateto") + "'";
			}
