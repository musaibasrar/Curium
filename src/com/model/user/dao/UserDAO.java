	...
	public int getNoOfStudents(int branchId) {
		 java.util.List<Student> results = new ArrayList<Student>();
	        ...
	        try {
	            ...
	            results = (java.util.List<Student>) session.createQuery("FROM Student s where s.archive = 0 AND s.passedout = 0 AND (s.remarks = 'approved' OR s.remarks='admin') AND s.droppedout = 0 AND s.branchid="+branchId+" order by s.admissionnumber DESC").list();
	...
	public int getNoOfStudentsOne(String classStudying, int branchId) {
		 java.util.List<Student> results = new ArrayList<Student>();
	        ...
	        try {
	            ...
	            results = (java.util.List<Student>) session.createQuery("From Student s where s.classstudying LIKE '"+classStudying+" %' OR s.classstudying = '"+classStudying+"'  AND s.archive = 0 AND s.passedout = 0 AND (s.remarks = 'approved' or s.remarks='admin') AND s.droppedout = 0 AND s.branchid="+branchId+" Order by s.admissionnumber ASC").list();
	            noOfRecords = results.size();
	            transaction.commit();
